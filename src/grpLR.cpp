...
void grp_sth_LR(double *sub_x, int *pp, double *iilambda0, double *gnorm)
{
    int k,p;
...
void get_LOR_EP(double *e, double *ep, double *A, double *x, int *xa_idx, int *nn, int *dd, int* pp, int *mm)
{
...
void get_grad_LR(double *g, double *A, double *ep, double *z, int *nn, int *mm)
{
    int i,j,b_idx;
    int n,m;
    n = *nn;
    ...
    for(j=0;j<(m+1);j++){
        ...
        b_idx = n*j;
        for(i=0;i<n;i++)
            g[j] = g[j] + ep[i]*A[b_idx+i];
    }
}

void get_base_LR(double *H0, double *z, double *x, double *e, int *xa_idx, int *nn, int *dd, int *pp, int *mm)
{
    int i,j,k,g_idx;
    int n,m,d,p;
    n = *nn;
...
void grpLR(double *A, double *lambda, int *nnlambda, double *LL0, int *nn, int *dd, int *pp, double *xx, double *aa0, int *mmax_ite, double *tthol, double *aalpha, double *z, int *df, double *func_norm)
{
    ...
    double *e,*ep,*g,*x0,*x1,*y1,*y2,t1,t2;
    ...
    for(counter=0;counter<nlambda;counter++){
        ...
        get_LOR_EP(e,ep,A,y1,ya_idx,&n,&d,&p,&m);
    ...
    */
        ...
        get_base_LR(&H0, z, y1, e, ya_idx, &n, &d, &p, &m);
        ...
        while(tracking==1){
            ...
            for(j=0;j<d;j++){
                grp_sth_LR(x1+j*p,&p,&ilambda0,&gnorm);
                if(gnorm!=0)
                    xa_idx[j] = 1;
                else
                    xa_idx[j] = 0;
            ...
            get_LOR_EP(e,ep,A,x1,xa_idx,&n,&d,&p,&m);
        
            get_base_LR(&H, z, x1, e, xa_idx, &n, &d, &p, &m);
        ...
        get_base_LR(&Hx0, z, x1, e, xa_idx, &n, &d, &p, &m);
